# -*- coding: utf-8 -*-
"""
Created on Sat Nov  5 21:40:39 2016
...
for word in dictionary:
    ratio[word] = ngCountSpam[word]/ngCountHam[word]
...
#Saving the data
with open(projectPath+"5foldCV/"+str(NUMBER)+"/dictionaryFSBG"+str(ThOccurance)+".txt",'wu') as f:
    f.write('\n'.join('%s' % str(x) for x in dictionaryFS))

with open(projectPath+"5foldCV/"+str(NUMBER)+"/ratioBG"+str(ThOccurance)+".txt",'wu') as f:
    f.write('\n'.join('%s' % str(x) for x in sorted_ratio))


with open(projectPath+"/5foldCV/"+str(NUMBER)+"/dictionaryBGFS"+str(ThOccurance)+".pickle", 'wb') as handle:
