...
#Loading dictionary
with open("/home/ozgur/Desktop/NLP Project/5foldCV/"+str(NUMBER)+"/dictionary"+str(ThOccurance)+".pickle", 'rb') as handle:
...
for mailText in glob.glob(os.path.join(pathCorpusTrain, '*.txt')):
    ...
    wordVec = np.zeros([dicSize+dicSizeBG,1])
    indices = []
    ...
    with open(mailText,'ru') as f:
        ...
        for ng in bigrams:
            if ng in dictionaryBG:
                indices.append(dicSize+dictionaryBG.index(ng))
